...
### Version 1.1 · Works with Content Spec + Narration Flow Spec · April 2026
...
## SECTION 0A: GENERATION CONTROLS (NEW REQUIRED ROOT LAYER)

Generation controls make the runtime anti-drift model explicit. This is the
primary generation-control layer for standalone stateless execution.

### 0A.1 Generation controls object (required)

```json
"generation_controls": {
  "intent_profile": ["relatable", "useful", "grounded"],
  "available_pools": {
    "content_mode_pool": ["story", "analysis", "guide", "reflection", "comparison"],
    "angle_pool": ["personal_experience", "practical_breakdown", "myth_busting"],
    "tone_pool": ["honest", "calm", "grounded"],
    "hook_style_pool": ["observation", "practical_problem", "small_truth"]
  },
...
|---|---|---|---|
| intent_profile | array | YES | 1+ descriptors defining the intended reader value |
| available_pools.content_mode_pool | array | YES | Allowed article-level macro modes |
| available_pools.angle_pool | array | YES | Allowed primary lenses |
| available_pools.tone_pool | array | YES | Allowed voice/tone values |
| available_pools.hook_style_pool | array | YES | Allowed opening hook families |
| selected_runtime_values.content_mode | string | YES | Must belong to content_mode_pool |
| selected_runtime_values.angle | string | YES | Must belong to angle_pool |
| selected_runtime_values.tone | string | YES | Must belong to tone_pool |
| selected_runtime_values.hook_style | string | YES | Must belong to hook_style_pool |
| runtime_override.used | boolean | YES | true only when safety or input incompatibility forces override |
| runtime_override.reason | string or null | YES | non-null only when used=true |

Validation:
- any selected value not present in its pool → REJECT
- runtime_override.used=true and reason null/empty → REJECT
- runtime_override.used=false and reason non-null → REJECT

`content_mode` is article-level and must not be confused with section-level `content_option`.

## SECTION 0B: THEME METADATA (SECONDARY EDITORIAL LAYER)

Theme metadata is an **additive secondary** root-level layer for editorial packaging/classification. It must not override generation_controls or change any existing safety or structure requirements.

### 0B.1 Theme object (required)
...
### 0B.2 Theme override object (required)
...
Sensitive advice categories such as health, mental wellness, therapy, relationship, and social counselling must set `has_sensitive_advice_content` to false.
...
| Sensitive advice flag | has_sensitive_advice_content must be false | REJECT |
| Exclusive word tier | is_short_read and is_medium_content not both true | REJECT |
| Generation controls present | generation_controls exists and selected runtime values are valid | REJECT |
